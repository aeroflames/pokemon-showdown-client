...
export const Dex = new class implements ModdedDex {
	...
	resourcePrefix = 'https://raw.githubusercontent.com/aeroflames/pokemon-sprites/main/'

	// fxPrefix = (() => {
	// 	const protocol = (window.document?.location?.protocol !== 'http:') ? 'https:' : '';
	// 	return `${protocol}//${window.Config ? Config.routes.client : 'play.pokemonshowdown.com'}/fx/`;
	// })();
	fxPrefix = 'https://raw.githubusercontent.com/aeroflames/pokemon-sprites/main/fx/'
	...
	forFormat(format: string) {
		...
		if (dex.gen === 9 && formatid.includes('legendszadraft')) {
			dex = Dex.mod('gen9legendszadraft' as ID);
